define("echarts/chart/chord", ["require", "./base", "zrender/shape/Text", "zrender/shape/Line", "zrender/shape/Sector", "../util/shape/Ribbon", "../util/shape/Icon", "zrender/shape/BezierCurve", "../config", "../util/ecData", "zrender/tool/util", "zrender/tool/vector", "../data/Graph", "../layout/Chord", "../chart"], function (e) {
    ...
    var d = e("../util/ecData"), c = e("zrender/tool/util"), m = e("zrender/tool/vector"), p = e("../data/Graph"), u = e("../layout/Chord");
    return t.prototype = {
        type: h.CHART_TYPE_CHORD, _init: function () {
            ...
            if (t.length) {
                var l = t[0];
                if (!i.ribbonType) {
                    var h = i.minRadius, d = i.maxRadius, c = 1 / 0, m = -(1 / 0);
                    l.eachNode(function (e) {
                        m = Math.max(e.layout.size, m), c = Math.min(e.layout.size, c)
                    });
                    var p = (d - h) / (m - c);
                    l.eachNode(function (e) {
                        var t = this._getNodeQueryTarget(i, e), n = this.query(t, "symbolSize");
                        e.layout.size = m === c ? n || c : n || (e.layout.size - c) * p + h
                    }, this)
                }
                var g = new u;
                ...
                if (i.ribbonType) {
                    ...
                    for (var o = 0, U = 0; o < e.length; o++)this.selectedMap[e[o].name] && this._buildRibbons(e, o, t[U++], i);
                    i.showScale && this._buildScales(i, 0, l)
                } else {
                    this._buildNodeIcons(i, 0, l, i, t), V && this._buildLabels(i, 0, l, i, t);
                    for (var o = 0, U = 0; o < e.length; o++)this.selectedMap[e[o].name] && this._buildEdgeCurves(e, o, t[U++], i, l)
                }
            ...
            for (var i = new p(!0), n = e.data || e.nodes, a = 0, o = n.length; o > a; a++) {
                var r = n[a];
                if (r && !r.ignore) {
            ...
            i.eachNode(function (i) {
                var h = this._getNodeCategory(n, i.data), c = this.getColor(h ? h.name : i.id), m = i.layout.startAngle / Math.PI * 180 * l, p = i.layout.endAngle / Math.PI * 180 * l, u = new o({
            ...
            i.eachNode(function (i) {
                var o = i.layout.startAngle, l = i.layout.endAngle, h = (o + l) / 2, c = r * Math.cos(h), m = r * Math.sin(h), p = this._getNodeQueryTarget(n, i.data), u = this._getNodeCategory(n, i.data), g = this.deepQuery(p, "itemStyle.normal.color");
            ...
            i.eachNode(function (e) {
                ...
                r ? (U.rotation = p ? d : Math.PI + d, U.style.x = p ? h[1] + g : -h[1] - g, U.style.y = 0, U.position = l.slice()) : (U.style.x = V[0], U.style.y = V[1]), U.style.textColor = this.deepQuery([e.data, a], "itemStyle.normal.label.textStyle.color") || "#fff", U.style.textFont = this.getFont(this.deepQuery([e.data, a], "itemStyle.normal.label.textStyle")), U = new n(U), this.shapeList.push(U), e.labelShape = U
            }, this)
            ...
            i.eachEdge(function (l, h) {
                var c, m = i.getEdge(l.node2, l.node1);
                if (m && !l.shape) {
                    if (m.shape)return void(l.shape = m.shape);
                    var p = l.layout.startAngle / Math.PI * 180, u = l.layout.endAngle / Math.PI * 180, g = m.layout.startAngle / Math.PI * 180, V = m.layout.endAngle / Math.PI * 180;
                    c = this.getColor(1 === e.length ? l.layout.weight <= m.layout.weight ? l.node1.id : l.node2.id : a.name);
                    var U, y, f = this._getEdgeQueryTarget(a, l.data), _ = this._getEdgeQueryTarget(a, l.data, "emphasis"), b = new r({
                        zlevel: this.getZlevelBase(),
                        z: this.getZBase(),
                        style: {
                            x: o[0],
                    ...
                    l.layout.weight <= m.layout.weight ? (U = m.node1, y = m.node2) : (U = l.node1, y = l.node2), d.pack(b, a, t, l.data, null == l.rawIndex ? h : l.rawIndex, l.data.name || U.id + "-" + y.id, U.id, y.id), this.shapeList.push(b), l.shape = b
                }
            }, this)
        }, _buildEdgeCurves: function (e, t, i, n, a) {
            var o = e[t], r = this.parseCenter(this.zr, n.center);
            i.eachEdge(function (e, i) {
                var n = a.getNodeById(e.node1.id), s = a.getNodeById(e.node2.id), h = n.shape, c = s.shape, m = this._getEdgeQueryTarget(o, e.data), p = this._getEdgeQueryTarget(o, e.data, "emphasis"), u = new l({
                    zlevel: this.getZlevelBase(),
                    z: this.getZBase(),
                    style: {
                        xStart: h.position[0],
            ...
            var o, r, s = e.clockWise, l = this.parseCenter(this.zr, e.center), h = this.parseRadius(this.zr, e.radius), d = s ? 1 : -1, c = 0, p = -(1 / 0);
            e.showScaleText && (i.eachNode(function (e) {
                var t = e.data.value;
                t > p && (p = t), c += t
            }), p > 1e10 ? (o = "b", r = 1e-9) : p > 1e7 ? (o = "m", r = 1e-6) : p > 1e4 ? (o = "k", r = .001) : (o = "", r = 1));
            var u = c / (360 - e.padding);
            i.eachNode(function (t) {
                for (var i = t.layout.startAngle / Math.PI * 180, c = t.layout.endAngle / Math.PI * 180, p = i; ;) {
                    if (s && p > c || !s && c > p)break;
                    var g = p / 180 * Math.PI, V = [Math.cos(g), Math.sin(g)], U = m.scale([], V, h[1] + 1);
                    m.add(U, U, l);
                    var y = m.scale([], V, h[1] + this.scaleLineLength);
                    m.add(y, y, l);
                    var f = new a({
                        zlevel: this.getZlevelBase(),
                        z: this.getZBase() - 1,
                    ...
                    this.shapeList.push(f), p += d * this.scaleUnitAngle
                }
    ...
    return t.prototype = {
        type: "ribbon", buildPath: function (e, t) {
            ...
            n.begin(e);
            var a = t.x, o = t.y, r = t.r, s = t.source0 / 180 * Math.PI, l = t.source1 / 180 * Math.PI, h = t.target0 / 180 * Math.PI, d = t.target1 / 180 * Math.PI, c = a + Math.cos(s) * r, m = o + Math.sin(s) * r, p = a + Math.cos(l) * r, u = o + Math.sin(l) * r, g = a + Math.cos(h) * r, V = o + Math.sin(h) * r, U = a + Math.cos(d) * r, y = o + Math.sin(d) * r;
            n.moveTo(c, m), n.arc(a, o, t.r, s, l, !i), n.bezierCurveTo(.7 * (a - p) + p, .7 * (o - u) + u, .7 * (a - g) + g, .7 * (o - V) + V, g, V), (t.source0 !== t.target0 || t.source1 !== t.target1) && (n.arc(a, o, t.r, h, d, !i), n.bezierCurveTo(.7 * (a - U) + U, .7 * (o - y) + y, .7 * (a - c) + c, .7 * (o - m) + m, c, m))
        }, getRect: function (e) {
    ...
    return i.Node = n, i.Edge = a, i.fromMatrix = function (e, t, n) {
        if (t && t.length && t[0].length === t.length && e.length === t.length) {
            ...
            for (var r = 0; a > r; r++)for (var l = r; a > l; l++) {
                var h = t[r][l];
                if (0 !== h) {
                    var d = o.nodes[r], c = o.nodes[l], m = o.addEdge(d, c, {});
                    if (m.data.weight = h, r !== l && n && t[l][r]) {
                        var p = o.addEdge(c, d, {});
                        p.data.weight = t[l][r]
                    }
                }
    ...
    e.prototype.run = function (e) {
        ...
        if (n) {
                ...
                for (var c = 0, m = 0; m < e.length; m++) {
                    ...
                    if (u) {
                        ...
                        for (var g = this.directed ? u.outEdges : u.edges, V = 0; V < g.length; V++) {
                            var U = g[V], y = U.layout.weight;
                            d.subGroups.push({weight: y, edge: U, graph: p}), c += y
                        }
                    }
                }
                s += d.size;
                for (var f = d.size / c, V = 0; V < d.subGroups.length; V++)d.subGroups[V].weight *= f;
            ...
            for (var f = (2 * Math.PI - this.padding * o) / s, _ = this.startAngle, b = this.clockWise ? 1 : -1, l = 0; o > l; l++) {
                var d = r[l];
