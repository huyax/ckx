define("echarts/chart/eventRiver", ["require", "./base", "../layout/eventRiver", "zrender/shape/Polygon", "../component/axis", "../component/grid", "../component/dataZoom", "../config", "../util/ecData", "../util/date", "zrender/tool/util", "zrender/tool/color", "../chart"], function (e) {
    ...
    var n = e("./base"), i = e("../layout/eventRiver"), a = e("zrender/shape/Polygon");
    ...
    return t.prototype = {
        type: o.CHART_TYPE_EVENTRIVER, _buildShape: function () {
            ...
            for (var t = this.component.legend, n = [], a = 0; a < e.length; a++)if (e[a].type === this.type) {
            ...
            i(n, this._intervalX, this.component.grid.getArea()), this._drawEventRiver(), this.addShapeList()
            ...
            for (var e, t, n = this.series, i = 0, a = n.length; a > i; i++)if (n[i].type === this.type) {
                ...
                for (var o = 0, r = n[i].data.length; r > o; o++) {
                    t = n[i].data[o].evolution;
            ...
            for (var e = this.series, t = 0; t < e.length; t++) {
                var n = e[t].name || "";
                if (e[t].type === this.type && this.selectedMap[n])for (var i = 0; i < e[t].data.length; i++)this._drawEventBubble(e[t].data[i], t, i)
            }
        }, _drawEventBubble: function (e, t, n) {
            var i = this.series, o = i[t], s = o.name || "", l = o.data[n], m = [l, o], V = this.component.legend, U = V ? V.getColor(s) : this.zr.getColor(t), d = this.deepMerge(m, "itemStyle.normal") || {}, p = this.deepMerge(m, "itemStyle.emphasis") || {}, c = this.getItemStyleColor(d.color, t, n, l) || U, u = this.getItemStyleColor(p.color, t, n, l) || ("string" == typeof c ? h.lift(c, -.2) : c), y = this._calculateControlPoints(e), g = {
            ...
            g = new a(g), this.addLabel(g, o, l, e.name), r.pack(g, i[t], t, i[t].data[n], n, i[t].data[n].name), this.shapeList.push(g)
        }, _calculateControlPoints: function (e) {
            var t = this._intervalX, n = e.y, i = e.evolution, a = i.length;
            if (!(1 > a)) {
                for (var o = [], r = [], s = 0; a > s; s++)o.push(i[s].timeScale), r.push(i[s].valueScale);
                var l = [];
                l.push([o[0], n]);
                var s = 0;
                for (s = 0; a - 1 > s; s++)l.push([(o[s] + o[s + 1]) / 2, r[s] / -2 + n]);
                for (l.push([(o[s] + (o[s] + t)) / 2, r[s] / -2 + n]), l.push([o[s] + t, n]), l.push([(o[s] + (o[s] + t)) / 2, r[s] / 2 + n]), s = a - 1; s > 0; s--)l.push([(o[s] + o[s - 1]) / 2, r[s - 1] / 2 + n]);
                return l
            }
    ...
    function e(e, o, r) {
        ...
        for (var g = n(Math.floor(c), Math.ceil(u)), b = 0, V = 0; V < e.length; V++)for (var U = 0; U < e[V].data.length; U++) {
            var f = e[V].data[U];
            f.time = [], f.value = [];
            ...
            var k = l(f.value, Math.max.apply(Math, f.value)), x = i(g, f.time[k], f.time[k + 1]), p = 0;
            for (f.y = x + f.value[k] / 2 + h, p = 0; p < f.time.length - 1; p++) {
                var _ = i(g, f.time[p], f.time[p + 1]);
                f.y - f.value[p] / 2 - h < _ && (f.y = _ + f.value[p] / 2 + h)
            }
            var _ = i(g, f.time[p], f.time[p] + m);
    ...
    function t(e, t, n, i) {
        for (var a = t.y, o = (t.height - i) / n, r = 0; r < e.length; r++) {
            e[r].y = e[r].y * o + a;
    ...
    function i(e, t, n) {
        ...
        if (t == e.left && n == e.right)o = e.maxValue; else if (a >= n && null != e.leftChild)o = i(e.leftChild, t, n); else if (t >= a && null != e.rightChild)o = i(e.rightChild, t, n); else {
            var r = 0, s = 0;
            null != e.leftChild && (r = i(e.leftChild, t, a)), null != e.rightChild && (s = i(e.rightChild, a, n)), o = r > s ? r : s
        }
        return o
    }
